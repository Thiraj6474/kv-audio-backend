...
export async function createOrder(req,res){
    ...
    if(lastOrder.length == 0 ){
        ...
        const currentNumber = lastOrderNumber + 1 //66
        const formatedNumber = String(currentNumber).padStart(4,'0');
        orderInfo.orderId = "ORD" + formatedNumber;
    }
    let oneDayCost = 0;
    for(let i=0; i<data.orderedItems.length; i++){
        try{
            ...
            if(product == null){
                res.status(404).json({
                    message: "Product with " +data.orderedItems[i].key + " not found"
                })
                return
            }
            if(product.availability == false){
                res.status(400).json({
                    message: "Product with " +data.orderedItems[i].key + " is not available"
                })
                return
            }
